...
get_build_report() {
  ...
  if [ "$STAGE_NAME" == 'run-test' -a  "$STATUS" == 'success' ]; then
    MESSAGE_TEXT="Test Phase Passed! :jenkins: :smiley:"
    COLOR="good"

  elif [ "$STAGE_NAME" == 'run-test' -a  "$STATUS" == 'fail' ]; then
    MESSAGE_TEXT="Test Phase Failed :jenkins: :scream:"
    COLOR="danger"
    REBUILD_URL="https://jenkins.andela.com/blue/organizations/jenkins/converge-slack-integration/detail/${CURRENT_BRANCH}/${BUILD_NUMBER}/pipeline"
    ACTION_BUTTON="$(echo \
          "{\"type\": \"button\", \"text\": \"Rebuild\", \"url\": \"${REBUILD_URL}\"}", \
      )"

  elif [ "$STAGE_NAME" == 'build-deployment-image' -a "$STATUS" == 'success' ]; then
    MESSAGE_TEXT="Build Phase Succeeded  :jenkins: :rocket:"
    COLOR="good"

  elif [ "$STAGE_NAME" == 'build-deployment-image' -a  "$STATUS" == 'fail' ]; then
  ...
  elif [ "$STAGE_NAME" == 'deploy-to-GKE' -a "$STATUS" == 'success' ]; then
    MESSAGE_TEXT="Deployment Phase Succeeded :jenkins: :rocket:"
    COLOR="good"

  elif [ "$STAGE_NAME" == 'deploy-to-GKE' -a  "$STATUS" == 'fail' ]; then
    MESSAGE_TEXT="Deployment Phase Failed  :jenkins: :scream:"
    REBUILD_URL="https://jenkins.andela.com/blue/organizations/jenkins/converge-slack-integration/detail/${CURRENT_BRANCH}/${BUILD_NUMBER}/pipeline"
    ACTION_BUTTON="$(echo \
          "{\"type\": \"button\", \"text\": \"Rebuild\", \"url\": \"${REBUILD_URL}\"}", \
  ...
  COMMIT_LINK="https://github.com/andela/converge-slack-integration/commit/${GIT_COMMIT}"
  IMAGE_TAG="$(git rev-parse --short HEAD)"
  JENKINS_WORKFLOW_URL="https://jenkins.andela.com/blue/organizations/jenkins/converge-slack-integration/detail/${CURRENT_BRANCH}/${BUILD_NUMBER}/pipeline"
...
send_notification() {
  ...
  "payload={
      ...
      \"attachments\": [{
          ...
          \"author_link\": \"https://github.com/andela/mrm_slack_integration/tree/${BRANCH_NAME}\",
